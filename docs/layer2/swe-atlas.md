...
3. 形态。`task`、`dataset`、`environment`、`verifier`、`agent`。示例直接调用 `harbor run`。公开材料没有单独的 Harbor adapter。

4. 与 Harbor 距离。轻适配。这个 commit 有 Harbor task 目录，示例脚本是 `harbor run`。README 钉的是 Harbor v0.18.0，不是 0.23.0，所以不是原生可接。三轨评分都要另调模型。`data/rf` 有 70 个目录，没有 `dataset.toml`。
...
8. 迁入代价。中。任务目录已经有 `task.toml`，示例就是 `harbor run`。还要准备评判模型用的密钥。Refactoring 没有 `dataset.toml`。README 钉的是 Harbor v0.18.0，不是 0.23.0。
