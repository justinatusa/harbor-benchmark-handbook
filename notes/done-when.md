# Done-when 勾选（过程记录，不是读者手册）

2026-09-25 审查前，这份勾选放在 `docs/done-when.md`，并且全部打了勾。审查发现其中若干勾选与当时正文不符：轻适配把没有 Harbor task 目录的题算进去，`BaseInstalledAgent` 被写成安装包里不存在，转化清单要求打开私有 `notes/`。那些勾选不能当成完成证明。现行读者正文以 `docs/` 为准。公开树里的 [`docs/done-when.md`](../docs/done-when.md) 是同一份检查清单，并写明勾选不等于收工。
